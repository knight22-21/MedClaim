...
async def create_workflow(
    ...
    try:
        ...
        logger.info("approval.workflow.created | workflow_id=%s name=%s created_by=%s",
                    result.data[0]["id"], name, created_by)
...
async def get_workflow(workflow_id: str) -> dict[str, Any] | None:
    ...
    try:
        # Get workflow
        workflow_result = client.table("approval_workflows").select("*").eq("id", workflow_id).execute()
        ...
        # Get steps
        steps_result = client.table("approval_chain_steps").select("*").eq("workflow_id", workflow_id).order("step_order").execute()
...
async def update_workflow(
    ...
    try:
        ...
        result = client.table("approval_workflows").update(update_data).eq("id", workflow_id).execute()
        ...
        logger.error("approval.workflow.update_failed | workflow_id=%s error=%s", workflow_id, str(e))
        raise
...
async def delete_workflow(workflow_id: str) -> None:
    ...
    try:
        ...
        logger.error("approval.workflow.delete_failed | workflow_id=%s error=%s", workflow_id, str(e))
        raise
...
async def add_workflow_step(
    ...
    try:
        ...
        logger.info("approval.workflow.step_added | workflow_id=%s step_order=%s role=%s",
                    workflow_id, step_order, required_role)
        
        return result.data[0]
        
    except Exception as e:
        logger.error("approval.workflow.step_add_failed | workflow_id=%s error=%s", workflow_id, str(e))
        raise
...
async def update_workflow_step(
    ...
    try:
        ...
        result = client.table("approval_chain_steps").update(update_data).eq("id", step_id).execute()
...
async def process_approval_action(
    ...
    try:
        ...
        approval_result = client.table("claim_approvals").select("*").eq("claim_id", claim_id).execute()
        ...
        if action == "approved":
            ...
            workflow_steps_result = client.table("approval_chain_steps").select("*").eq("workflow_id", approval["workflow_id"]).order("step_order").execute()
            ...
            if next_step > len(workflow_steps):
                ...
                # Update claim status
                client.table("claims").update({"status": "READY_FOR_SUBMISSION"}).eq("id", claim_id).execute()
            else:
                # Move to next step
                ...
                # Update claim step
                client.table("claims").update({"current_approval_step": next_step}).eq("id", claim_id).execute()
            ...
            # Update claim status
            client.table("claims").update({"status": "CORRECTION_PENDING"}).eq("id", claim_id).execute()
        
        elif action == "escalated":
            # Find escalation role for current step
            current_step_result = client.table("approval_chain_steps").select("*").eq("workflow_id", approval["workflow_id"]).eq("step_order", approval["current_step"]).execute()
            ...
            if current_step_data and current_step_data["escalation_to_role"]:
                ...
                escalation_step_result = client.table("approval_chain_steps").select("*").eq("workflow_id", approval["workflow_id"]).eq("required_role", current_step_data["escalation_to_role"]).execute()
                if escalation_step_result.data:
                    escalation_step = escalation_step_result.data[0]
                    timeout_at = datetime.utcnow() + timedelta(hours=escalation_step["timeout_hours"])
                    update_data = {
                        "current_step": escalation_step["step_order"],
                        "timeout_at": timeout_at.isoformat(),
                        "status": "escalated",
                    }
                    client.table("claims").update({"current_approval_step": escalation_step["step_order"]}).eq("id", claim_id).execute()
                else:
        ...
        result = client.table("claim_approvals").update(update_data).eq("id", approval["id"]).execute()
        
        logger.info("approval.claim.action_processed | claim_id=%s action=%s approver_id=%s",
                    claim_id, action, approver_id)
...
async def get_claim_approval(claim_id: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        # Get history
        history_result = client.table("approval_history").select("*").eq("claim_approval_id", approval["id"]).order("created_at").execute()
...
async def check_timeout_escalations() -> list[dict[str, Any]]:
    ...
    try:
        ...
        result = client.table("claim_approvals").select("*").eq("status", "pending").lt("timeout_at", now).execute()
        ...
        for approval in timed_out:
            try:
                # Get current step
                step_result = client.table("approval_chain_steps").select("*").eq("workflow_id", approval["workflow_id"]).eq("step_order", approval["current_step"]).execute()
                ...
                if step_data and step_data["escalation_to_role"]:
                    ...
                    escalation_step_result = client.table("approval_chain_steps").select("*").eq("workflow_id", approval["workflow_id"]).eq("required_role", step_data["escalation_to_role"]).execute()
                    if escalation_step_result.data:
                        escalation_step = escalation_step_result.data[0]
                        timeout_at = datetime.utcnow() + timedelta(hours=escalation_step["timeout_hours"])
                        ...
                        logger.info("approval.claim.timeout_escalated | claim_id=%s", approval["claim_id"])
                
            except Exception as e:
                logger.error("approval.claim.escalation_failed | claim_id=%s error=%s", approval["claim_id"], str(e))
