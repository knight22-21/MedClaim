...
async def create_comment(
    ...
    try:
        ...
        logger.info("comment.created | comment_id=%s claim_id=%s user_id=%s parent_id=%s",
                    result.data[0]["id"], claim_id, user_id, parent_id)
        
        return result.data[0]
        
    except Exception as e:
        logger.error("comment.create_failed | claim_id=%s user_id=%s error=%s", claim_id, user_id, str(e))
        raise
...
async def update_comment(
    ...
    try:
        ...
        result = client.table("comments").update({"content": content}).eq("id", comment_id).execute()
