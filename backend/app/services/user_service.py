...
async def create_user(
    ...
    try:
        ...
        logger.info(
            "user.created | user_id=%s email=%s role=%s created_by=%s",
            user_id,
            email,
            role,
            created_by,
...
async def list_users(
    role: str | None = None, limit: int = 100, offset: int = 0
) -> list[dict[str, Any]]:
...
async def update_user(
    ...
    try:
        ...
        logger.info(
            "user.updated | user_id=%s updated_by=%s changes=%s",
            user_id,
            updated_by,
...
async def invite_user(
    ...
    logger.info("user.invited | user_id=%s email=%s invited_by=%s", user["id"], email, invited_by)
