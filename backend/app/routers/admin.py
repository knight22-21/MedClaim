...
async def create_new_user(
    request: CreateUserRequest, current_user: dict[str, Any] = Depends(require_admin)
) -> APIResponse[UserProfileResponse]:
...
async def update_user_by_id(
    user_id: str, request: UpdateUserRequest, current_user: dict[str, Any] = Depends(require_admin)
) -> APIResponse[UserProfileResponse]:
...
async def invite_new_user(
    request: InviteUserRequest, current_user: dict[str, Any] = Depends(require_admin)
) -> APIResponse[dict]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to invite user"
        )
